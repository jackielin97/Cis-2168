...
import assign2_template.VideoGame;
...
public class SimpleLinkedList {
	...
	public boolean removeByValue(int item) {
		// add your own code
		boolean itemh = contains(item);
		// Hint:
		// call your indexOf(...) to locate the item,
		int nodeindex=indexOf(item);
		...
		if(itemh == true) {
			...
			if(nodeindex==0) {
				...
				Node prenode =getnode(nodeindex-1);
...
//		return 2;
	...
	public int indexOf(int item) {
		...
		while (current != null) {
			...
			if (Integer.compare(data, item) == 0) {
				results = count;
	...
	// return a string that contains all integers (in the original sequence) in the
	// linked list.
	@Override
	public String toString() {
